...
class App:
    def __init__(self):
        ...
        self.obj_mesh = Mesh("modelos3D/intestino.obj")
        ...
        #Configuração das perspectivas de projeção
    ...
    def mainLoop(self):
        ...
        while (running):
            ...
            self.obj.eulers[2] -= 0.25
            if self.obj.eulers[2] < 360:
                self.obj.eulers[2] += 360
            ...
            model_transform = pyrr.matrix44.multiply(
                ...
                m2=pyrr.matrix44.create_from_eulers(
                    eulers=np.radians(self.obj.eulers), dtype=np.float32
                )
            )
            model_transform = pyrr.matrix44.multiply(
                m1=model_transform, 
                m2=pyrr.matrix44.create_from_translation(
                    vec=np.array(self.obj.position),dtype=np.float32
                )
            )
